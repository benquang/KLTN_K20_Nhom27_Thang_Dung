...
def RF_Predictions(RFModel, X_test):

    predictions = RFModel.transform(X_test)
    
    return predictions
def RandomForest_Optimize_Func(matches_df, encoded_matches_label):
    df_measures = pd.DataFrame(columns = ['numTrees', 'maxDepth', 'accuracy', 'recall', 'f1_score', 'precision'])

    for numTrees in range(10, 50, 10):
        for maxDepth in range(5, 10):
            ...
            list_precision = []
            for i in range(0, 15):
                print("i = ", i, "numTrees = ", numTrees, "maxDepth = ", maxDepth)
                ...
                rf = RandomForestClassifier(numTrees=numTrees, maxDepth=maxDepth, featuresCol="Features", labelCol="Label")
            ...
            df_measures.to_csv('./measures/RandomForest_Optimize.csv', index = False)
    return df_measures
...
df_measures = RandomForest_Optimize_Func(matches_df, encoded_matches_label)
